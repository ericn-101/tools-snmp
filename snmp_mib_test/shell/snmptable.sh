...
#
# PURPOSE:  This shell script performs a snmptable using Net-SNMP and passed 
#           in args 
#
...
#
# CALL
#           ./snmptable-ng.sh -v "v3" -u "testerSHA"  -a "SHA" -b "snmptester123" -p "nopriv" -q "nopriv" -o "VENDOR-MIB::softwareVersionTable" -t "10.1.144.53"
#           ./snmptable-ng.sh -v "v2c" -r "vendor" -o "IF-MIB::ifXTable" -t "10.1.1.111"
#
...
then
  if (( $DEBUG == 1 )) 
  then
    echo -e "DEBUG:  Executing SNMP v2c MIB Table Command"
  fi
  snmptable -v 2c -c ${SNMP2CREAD} ${TGTHOST} ${MYOID} 
else
  if [ "$CAPPRIVTYPE" == "NOPRIV" ] 
  then
    if (( $DEBUG == 1 )) 
    then
      echo -e "DEBUG:  Executing SNMP v3 MIB Table Command AuthNoPriv"
    fi
    snmptable -v 3 -n "" -u ${SNMPUSER} -a ${CAPAUTHTYPE} -A ${AUTHKEY} -l authNoPriv ${TGTHOST} ${MYOID} 
  else
    if (( $DEBUG == 1 )) 
    then
      echo -e "DEBUG:  Executing SNMP v3 MIB Table Command AuthPriv"
    fi
    snmptable -v 3 -n "" -u ${SNMPUSER} -a ${CAPAUTHTYPE} -A ${AUTHKEY} -x ${CAPPRIVTYPE} -X ${PRIVKEY} -l authPriv ${TGTHOST} ${MYOID}
  fi
fi
