...
#
# PURPOSE:  This shell script performs a snmpWalk using Net-SNMP and passed 
#           in args 
#
...
#
# CALL
#           ./snmpwalk-ng.sh -v "v3" -u "testerSHA"  -a "SHA" -b "snmptester123" -p "nopriv" -q "nopriv" -o "VENDOR-MIB::vendor" -t "10.1.144.53"
#           ./snmpwalk-ng.sh -v "v2c" -r "vendor" -o "IF-MIB::ifDescr" -t "10.1.1.111"
#
...
then
  if (( $DEBUG == 1 )) 
  then
    echo -e "DEBUG:  Executing SNMP v2c MIB Walk Command"
  fi
  snmpwalk -v 2c -c ${SNMP2CREAD} ${TGTHOST} ${MYOID} 
else
  if [ "$CAPPRIVTYPE" == "NOPRIV" ] 
  then
    if (( $DEBUG == 1 )) 
    then
      echo -e "DEBUG:  Executing SNMP v3 MIB Walk Command AuthNoPriv"
    fi
    snmpwalk -v 3 -n "" -u ${SNMPUSER} -a ${CAPAUTHTYPE} -A ${AUTHKEY} -l authNoPriv ${TGTHOST} ${MYOID} 
  else
    if (( $DEBUG == 1 )) 
    then
      echo -e "DEBUG:  Executing SNMP v3 MIB Walk Command AuthPriv"
    fi
    snmpwalk -v 3 -n "" -u ${SNMPUSER} -a ${CAPAUTHTYPE} -A ${AUTHKEY} -x ${CAPPRIVTYPE} -X ${PRIVKEY} -l authPriv ${TGTHOST} ${MYOID} 
  fi
fi
