...
# ////////////////////////////////////////////////////////////////////////////
# SCRIPT:   snmpgetnext.sh
#
# AUTHOR:   Eric Nelson
#
# PLATFORM: Not Platform Dependent
#
# PURPOSE:  This shell script performs a snmpGetNext using Net-SNMP and passed 
#           in args 
#
...
#
# CALL
#           ./snmpgetnext-ng.sh -v "v3" -u "testerSHA"  -a "SHA" -b "snmptester123" -p "nopriv" -q "nopriv" -o "VENDOR-MIB::clusterVersion.0" -t "10.1.144.53"
#           ./snmpgetnext-ng.sh -v "v2c" -r "vendor" -o "SNMPv2-MIB::sysName.0" -t "10.1.1.111"
#
...
{
    echo -e "\nERROR: This shell script requires 8 command line arguments to set
           up the SNMP transaction with a remote target.\n"
    usage
}
...
then
  if (( $DEBUG == 1 )) 
  then
    echo -e "DEBUG:  Executing SNMP v2c MIB GetNext Command"
  fi
  snmpgetnext -v 2c -c ${SNMP2CREAD} ${TGTHOST} ${MYOID} 
else
  if [ "$CAPPRIVTYPE" == "NOPRIV" ] 
  then
    if (( $DEBUG == 1 )) 
    then
      echo -e "DEBUG:  Executing SNMP v3 MIB GetNext Command AuthNoPriv"
    fi
    snmpgetnext -v 3 -n "" -u ${SNMPUSER} -a ${CAPAUTHTYPE} -A ${AUTHKEY} -l authNoPriv ${TGTHOST} ${MYOID}
  else 
    if (( $DEBUG == 1 )) 
    then
      echo -e "DEBUG:  Executing SNMP v3 MIB GetNext Command AuthPriv"
    fi
    snmpgetnext -v 3 -n "" -u ${SNMPUSER} -a ${CAPAUTHTYPE} -A ${AUTHKEY} -x ${CAPPRIVTYPE} -X ${PRIVKEY} -l authPriv ${TGTHOST} ${MYOID}
  fi
fi
